...
from models import ExtractionBatch, Legibility, PersonRecord1950, to_gt_record
...
def call_gemini(client: genai.Client, model: str, system_prompt: str,
                user_prompt: str, image_bytes: bytes, mime_type: str) -> list[PersonRecord1950]:
    ...
    for attempt in range(1, MAX_RETRIES_API + 1):
        try:
            response = client.models.generate_content(
                model=model, contents=contents, config=_config(system_prompt),
            )
            parsed = response.parsed
            if isinstance(parsed, ExtractionBatch):
                return parsed.records
            data = json.loads(response.text)
            return ExtractionBatch.model_validate(data).records
        except Exception as e:
            last_err = e
            if attempt < MAX_RETRIES_API:
                time.sleep(2 ** attempt)
    raise RuntimeError(f"Gemini call failed after {MAX_RETRIES_API} attempts: {last_err}")
...
def extract_from_image(
    ...
    """Extract one census page. Returns (records, diagnostics)."""
    if client is None:
        client = get_client()
    system_prompt, user_prompt = load_prompt(year)

    page_bytes, page_mime = prepare_full_page(image_path)
    full_records = call_gemini(client, model, system_prompt, user_prompt, page_bytes, page_mime)
    ...
    def retry_fn(missing_lines: list[int]) -> list[PersonRecord1950]:
        ...
        return call_gemini(client, model, system_prompt, user_prompt, crop.image_bytes, crop.mime_type)

    merged, diagnostics = reconcile_page(
        per_source, expected_lines=expected_lines, retry_fn=retry_fn if use_crops else None
    )

    records = [to_gt_record(p) for p in merged]
    records = _post_process(records, year)
    return records, diagnostics.model_dump()
...
def process_sheet(image_path: str, year: int, output_path: str,
    ...
    client = get_client()
    records, diagnostics = extract_from_image(
